...
counts_dir <- here::here(res_dir, "counts/")
...
#list files
counts_sums <- list.files(path = counts_dir, pattern = '_featureCounts.tsv$',recursive = T)
...
counts_sum <- list()
un_counts <- list()
for(i in seq_along(counts_sums)){
  # Calculate unassigned counts
  un_counts[[i]] <- str_c(counts_dir, counts_sums[i], ".summary") %>%
    read_tsv(.,show_col_types = FALSE) %>%
    pivot_longer(cols = last(names(.)), names_to = "sample") %>% 
  ...
  # featureCounts stats
  counts_sum[[i]] <- str_c(counts_dir, counts_sums[i]) %>%
    read_tsv(skip = 1,show_col_types = FALSE) %>%
    pivot_longer(cols = last(names(.)), names_to = "sample") %>% 
    trim_path(paste0("_",index_map,".*\\.bam$")) %>%
    dplyr::mutate(name = str_c(Geneid, gene_name, sep = "|")) %>%
    dplyr::select(
      name, Length, sample,
      metric = gene_biotype,
      value
...
# bring it all together and pivot to fit format
counts_sum <- bind_rows(counts_sum,un_counts) 
...
sub_sum <- sub_sum %>% read_tsv(.,show_col_types = FALSE) %>% 
  select(sample,starts_with("aligned",ignore.case = T),starts_with("sub")) 

if(any(names(sub_sum) %in% "sub_group")){
  sub_sum <- sub_sum %>%
    dplyr::rename(grp = sub_group,
                  `Filtered reads` = starts_with("aligned",ignore.case = T), 
                  `Sampled reads` = starts_with("subsampled_")) %>% 
    separate(.,`Sampled reads`,c("Sampled reads","temp"),"\\(",extra="drop",fill="right",convert=T) %>% 
    separate(.,`Filtered reads`,c("Filtered reads","temp"),"\\(",extra="drop",fill="right",convert=T) %>%
    dplyr::select(-temp) 
} else {
  sub_sum <- sub_sum %>% 
    dplyr::rename(`Filtered reads` = starts_with("aligned",ignore.case = T)) %>% 
    separate(.,`Filtered reads`,c("Filtered reads","temp"),"\\(",extra="drop",fill="right",convert=T) %>% 
    dplyr::select(-temp) %>% 
