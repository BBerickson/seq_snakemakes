...
trim_sum <- trim_sum %>%
  ...
  separate(.,value,c("value","per"),"\\(",convert=T) %>% dplyr::select(-per) %>% 
  pivot_wider(names_from = "metric", values_from = "value") %>%
  mutate(Untrimmed = `Total_read_pairs_processed` - (`Pairs_that_were_too_short` + `Pairs_written_(passing_filters)`)) %>%
...
dup_sum <- dedup_sum %>%
  ...
  pivot_longer(cols = -sample, names_to = "metric") 
dup_sum <- separate(dup_sum,sample,c("sample","index"),paste0("_",plot_grps$INDEX_MAP)) 
...
counts_sums <- counts_sum %>%
  ...
  group_by(sample, metric) %>%
  summarize(value = sum(value), .groups = "drop")
counts_sums <- separate(counts_sums,sample,c("sample","index"),paste0("_",plot_grps$INDEX_SAMPLE)) 
...
# Subsampling stats
#mets <- c("Sampled_reads", "Reads_removed")
mets <- c("Sampled_reads")

sub_sums <- sub_sum %>%
  vroom(col_names = c("sample", "metric", "value")) 
  
sub_sums <- separate(sub_sums,sample,c("sample","index"),paste0("_",plot_grps$INDEX_SAMPLE),fill = "right") %>% 
  dplyr::select(-index)
sub_sums <- sub_sums %>%  pivot_wider(names_from = "metric", values_from = "value") %>%
  #mutate(`Reads_removed` = `Filtered_reads` - `Sampled_reads`) %>%
  pivot_longer(cols = -c(sample), names_to = "metric") %>%
  dplyr::filter(metric %in% mets)
