...
sample_df <- tibble(stack(plot_grpss$SAMPLES)) %>% 
  dplyr::rename(file=values,sampling_grp=ind) %>% 
  mutate(plot_grp=plot_grpss$PROJ,
         clr=rep(plot_grpss$COLORS[seq_along(plot_grpss$groups)],length(names(plot_grpss$SAMPLES))),
         sample=rep(plot_grpss$groups,length(names(plot_grpss$SAMPLES)))) %>% 
  separate(.,sampling_grp,c("name","rep"),sep="-",remove=F) %>% mutate(sample=paste0(sample,"-",rep)) %>% 
  dplyr::select(-name,-rep)

sample_df <- sample_df %>%
  separate(sample, into = c("sam", "rep"), sep = "-", remove = FALSE)
