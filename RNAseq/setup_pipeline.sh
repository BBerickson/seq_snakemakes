...
rsync -a "$EXTRACT_DIR/RNAseq/ReadMe.txt" .
rsync -a "$EXTRACT_DIR/workflow/$SMK_FILE" workflow/
rsync -a "$EXTRACT_DIR/workflow/$SMK_FILE2" workflow/
...
rsync -a "$EXTRACT_DIR/workflow/submit_scripts/$SUBMIT_GLOB1" . 2>/dev/null
rsync -a "$EXTRACT_DIR/workflow/submit_scripts/$SUBMIT_GLOB2" . 2>/dev/null
...
echo ""
echo "!!!STAR is not set up for mixed genome alignment!!!"
echo "Files created:"
