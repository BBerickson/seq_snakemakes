...
fc_sum     <- str_c(report_dir, proj, "_featureCounts.tsv")
sub_sum    <- str_c(report_dir, proj, "_subsample.tsv")
...
`FeatureCount` was used to count the number of reads over the GTF biotype's. If < 2% and not protein_coding, snRNA, snoRNA, or rRNA placed in Other category.

```{r "FC summary", fig.width = 16, fig.height = 10}
...
mets <- fc_sum %>% group_by(metric) %>% summarise(value=sum(value),.groups = "drop") %>% arrange(desc(value)) %>% pull(metric)
...
fc_sum %>%
  create_qc_bars(
    ...
    sam_lvls   = NULL,
    met_lvls   = mets,
    plot_cols  = replace(theme_colors[seq_along(mets)], is.na(theme_colors[seq_along(mets)]), "black"),
    lab_metric = NULL,
    n_rows     = result,
    lab_size   = 20
  ) +
...
```{r "Sub summary", fig.width = 16, fig.height = 10, results='asis', echo=FALSE}

# Subsampling stats
mets <- c("Sampled reads", "Reads removed")

if(file.exists(sub_sum)){
  cat("## Subsampling\n\n")
  cat("Libraries were subsampled so that downstream analysis is performed using an equal number of unique reads for sample groups. The number of sampled reads is displayed on each bar.\n\n")

sub_sum <- sub_sum %>%
  read_tsv(col_names = c("grp", "sample", "metric", "value"),show_col_types = FALSE) %>%
  pivot_wider(names_from = "metric", values_from = "value") %>%
